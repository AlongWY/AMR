    ...
    for i, m in enumerate(mapping):
        ...
        for key in weight_dict[current_node_pair]:
            if key.first == -1:
                ...
                if verbose:
                    print ("instance/attribute match", key.second, file=DEBUG_LOG)
                continue
            ...
            if first < i:
                ...
                match_num += key.second
                if verbose:
                    print("relation match with", key, key.second, file=DEBUG_LOG)
    if verbose:
            ...
            if candidate_mappings[i].count(nm):
                ...
                if verbose:
                    ...
                    if new_match_num != cur_match_num + mv_gain:
                        ...
                        print("Inconsistency in computing: move gain", cur_match_num,
                              mv_gain, new_match_num, file=ERROR_LOG)
        ...
        for j in range(i + 1, len(mapping)):
            ...
            if verbose:
                ...
                if new_match_num != cur_match_num + sw_gain:
                    ...
                    print ("Inconsistency in computing: swap gain", cur_match_num,
                           sw_gain, new_match_num, file=DEBUG_LOG)
